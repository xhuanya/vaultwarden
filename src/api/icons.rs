...
use rocket::{http::ContentType, http::Cookie, response::content::Custom, Route};
use tokio::{
...
#[get("/<domain>/icon.png")]
async fn icon(domain: String) -> Cached<Custom<Vec<u8>>> {
    const FALLBACK_ICON: &[u8] = include_bytes!("../static/images/fallback-icon.png");

    if !is_valid_domain(&domain) {
        warn!("Invalid domain: {}", domain);
        return Cached::ttl(
            Custom(ContentType::new("image", "png"), FALLBACK_ICON.to_vec()),
    ...
    match get_icon(&domain).await {
        Some((icon, icon_type)) => {
            Cached::ttl(Custom(ContentType::new("image", icon_type), icon), CONFIG.icon_cache_ttl())
        }
        _ => Cached::ttl(Custom(ContentType::new("image", "png"), FALLBACK_ICON.to_vec()), CONFIG.icon_cache_negttl()),
    }
}
...
async fn get_icon_url(domain: &str) -> Result<IconUrlResult, Error> {
    ...
    if let Ok(content) = resp {
        ...
        let mut limited_reader = stream_to_bytes_limit(content, 512 * 1024).await?.reader();
...
async fn get_page_with_cookies(url: &str, cookie_str: &str, referer: &str) -> Result<Response, Error> {
    ...
    client.send().await?.error_for_status().map_err(Into::into)
}
...
async fn download_icon(domain: &str) -> Result<(Vec<u8>, Option<&str>), Error> {
    ...
    for icon in icon_result.iconlist.iter().take(5) {
        if icon.href.starts_with("data:image") {
            ...
            match datauri.decode::<_, ()>(|bytes| Ok(body.extend_from_slice(bytes))) {
                Ok(_) => {
